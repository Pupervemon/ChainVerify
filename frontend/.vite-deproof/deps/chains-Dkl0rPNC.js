import { A as size, B as fromHex$1, Bt as InvalidChainIdError$1, E as fromString$1, F as toString$1, H as isEqual, I as trimLeft, It as parseAccount, J as toString, L as validate, M as toBigInt, P as toNumber, Q as BaseError, R as concat$1, S as from$4, T as fromNumber, V as fromString, Z as stringify, a as maxUint256, b as assert$1, c as formatTransactionRequest$1, d as getCallError, i as maxUint16, j as slice, k as random, r as assertRequest, s as defineTransactionRequest, u as extract, w as fromBytes$1, x as concat } from "./isAddressEqual-DXU8Mzf0.js";
import { C as isAddress, T as InvalidAddressError, _ as concatHex } from "./encodeFunctionData-YLQ8NBAq.js";
import { a as toBytes$2, b as pad, c as numberToHex$1, h as hexToNumber$1, p as hexToBigInt, r as hexToBytes, u as toHex, w as BaseError$1 } from "./stringify-Bm23iD_D.js";
import { A as isEqual$1, B as defineTransactionReceipt, C as fromHex$3, D as assert$2, F as sha256$1, G as getTransactionType, I as validate$1, L as sha256, N as toHex$1, O as checksum, P as keccak256, R as sha384, S as Field, U as formatLog, V as formatTransactionReceipt$1, W as getCode, Z as sha256$2, _ as toTuple$2, a as recoverPublicKey, at as toRlp, b as createCurve, c as toRpcList$1, ct as getAction, d as from$5, et as defineBlock, f as fromHex$2, g as toRpc$5, h as toHex$2, i as recoverAddress, j as validate$2, k as fromPublicKey$1, l as assert$3, m as fromTuple$2, nt as defineTransaction, o as verify$4, p as fromRpc$5, r as verifyHash, rt as formatTransaction$1, s as fromRpcList$1, t as AccountNotFoundError, u as extract$1, v as yParityToV, w as toHex$3, x as mapToCurveSimpleSWU, y as createHasher, z as sha512 } from "./account-r4vEt8f4.js";
import { f as chainConfig$3, g as serializeTransaction$1, m as formatters$1 } from "./sepolia-8_St3NiF.js";
import { n as extendSchema, t as defineChain } from "./defineChain-DOrIgftx.js";
import { a as chainConfig$4 } from "./katana-CvyhsLGQ.js";
...
* @returns Access list.
*/
function fromTupleList$1(accessList) {
	const list = [];
	for (let i = 0; i < accessList.length; i++) {
		const [address, storageKeys] = accessList[i];
		if (address) assert$2(address, { strict: false });
		list.push({
			address,
...
*/
function fromRpc$4(transaction, _options = {}) {
...
function getEncodable(value) {
	if (typeof value === "undefined") return {
		length: 1,
		encode: (cursor) => cursor.pushUint8(247)
	};
	if (value === null) return {
		length: 1,
		encode: (cursor) => cursor.pushUint8(246)
	};
...
(function(getEncodable) {
	...
	function number(value) {
		...
		if (positiveNumber <= 65535) return {
			length: 3,
			encode: (cursor) => {
				cursor.pushUint8(57);
		...
		if (positiveNumber <= 4294967295) return {
			length: 5,
			encode: (cursor) => {
				cursor.pushUint8(58);
...
function decodeCursor(cursor) {
	...
	switch (majorType) {
		case 0: return decodeCursor.readUnsignedInteger(cursor, additionalInfo);
		case 1: return decodeCursor.readNegativeInteger(cursor, additionalInfo);
		case 2: return decodeCursor.readByteString(cursor, additionalInfo);
		case 3: return decodeCursor.readTextString(cursor, additionalInfo);
		case 4: return decodeCursor.readArray(cursor, additionalInfo);
		case 5: return decodeCursor.readMap(cursor, additionalInfo);
		case 6: throw new UnsupportedTagError({ tag: additionalInfo });
		case 7: return decodeCursor.readSimpleOrFloat(cursor, additionalInfo);
		default: throw new InvalidMajorTypeError({ majorType });
	}
}
/** @internal */
(function(decodeCursor) {
	...
	function readNegativeInteger(cursor, additionalInfo) {
		return -1 - readLength(cursor, additionalInfo);
	}
	...
	function readByteString(cursor, additionalInfo) {
		if (additionalInfo === 31) {
			...
			let totalLength = 0;
			while (true) {
				if (cursor.inspectUint8() === 255) {
					cursor.readUint8();
					break;
				}
				const chunk = decodeCursor(cursor);
				if (!(chunk instanceof Uint8Array)) throw new InvalidIndefiniteLengthChunkError({ type: "byte string" });
				chunks.push(chunk);
	...
	function readTextString(cursor, additionalInfo) {
		if (additionalInfo === 31) {
			...
			while (true) {
				...
				const chunk = decodeCursor(cursor);
				if (typeof chunk !== "string") throw new InvalidIndefiniteLengthChunkError({ type: "text string" });
				chunks.push(chunk);
			}
			return chunks.join("");
		}
		const length = readLength(cursor, additionalInfo);
		return toString(cursor.readBytes(length));
	}
	...
	function readArray(cursor, additionalInfo) {
		...
		const length = readLength(cursor, additionalInfo);
		const result = [];
		for (let i = 0; i < length; i++) result.push(decodeCursor(cursor));
		return result;
	}
	...
	function readMap(cursor, additionalInfo) {
		if (additionalInfo === 31) {
			...
			while (true) {
				...
				result[keyStr] = decodeCursor(cursor);
			}
			return result;
		}
		const length = readLength(cursor, additionalInfo);
	...
	function readSimpleOrFloat(cursor, additionalInfo) {
		...
		if (additionalInfo === 25) return getFloat16(cursor.readUint16());
		...
		if (additionalInfo === 24) {
			...
			if (simpleValue < 32) throw new InvalidSimpleValueError({ value: simpleValue });
			return;
		}
		throw new InvalidAdditionalInfoError({ additionalInfo });
	}
...
//#endregion
//#region node_modules/ox/node_modules/@noble/curves/esm/nist.js
/**
...
//#endregion
//#region node_modules/ox/node_modules/@noble/curves/esm/p256.js
var secp256r1 = p256;
...
function verify$3(options) {
	...
	return secp256r1.verify(signature, payload instanceof Uint8Array ? payload : fromHex$1(payload), toHex$1(publicKey).substring(2), {
		lowS: true,
		...hash ? { prehash: true } : {}
	});
}
...
* ```
*/
var InvalidSerializedError$1 = class extends BaseError {
...
function verify$2(options) {
	...
	const authenticatorDataBytes = fromHex$1(authenticatorData);
	if (authenticatorDataBytes.length < 37) return false;
	if (rpId !== void 0) {
		if (!isEqual(authenticatorDataBytes.slice(0, 32), sha256$1(fromString$1(rpId), { as: "Bytes" }))) return false;
	}
	...
	if (!clientData.challenge || fromBytes$1(toBytes(clientData.challenge)) !== challenge) return false;
	if (origin !== void 0) {
		if (!(Array.isArray(origin) ? origin : [origin]).includes(clientData.origin)) return false;
	}
	return verify$3({
		hash: true,
		payload: concat$1(authenticatorDataBytes, sha256$1(fromString(clientDataJSON), { as: "Bytes" })),
		publicKey,
		signature
	});
}
...
var InvalidEip712TransactionError = class extends BaseError$1 {
	constructor() {
		super([
...
function assertEip712Transaction(transaction) {
	...
	if (paymaster && !paymasterInput) throw new BaseError$1("`paymasterInput` must be provided when `paymaster` is defined");
	if (!paymaster && paymasterInput) throw new BaseError$1("`paymaster` must be provided when `paymasterInput` is defined");
}
//#endregion
...
var BytecodeLengthExceedsMaxSizeError = class extends BaseError$1 {
...
var BytecodeLengthInWordsMustBeOddError = class extends BaseError$1 {
...
var BytecodeLengthMustBeDivisibleBy32Error = class extends BaseError$1 {
...
function hashBytecode(bytecode) {
	const bytecodeBytes = toBytes$2(bytecode);
	...
	const hash = toBytes$2(sha256$2(bytecodeBytes));
	...
	const bytecodeLengthPadded = pad(toBytes$2(bytecodeLengthInWords), { size: 2 });
...
var ancient8 = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$63]: { address: "0xB09DC08428C8b4EFB4ff9C0827386CDF34277996" } },
		portal: { [sourceId$63]: {
...
var ancient8Sepolia = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$62]: { address: "0x942fD5017c0F60575930D8574Eaca13BEcD6e1bB" } },
		portal: { [sourceId$62]: {
			address: "0xfa1d9E26A6aCD7b22115D27572c1221B9803c960",
			blockCreated: 4972908
		} },
		l1StandardBridge: { [sourceId$62]: {
...
var baseGoerli = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$58]: { address: "0x2A35891ff30313CcFa6CE88dcf3858bb075A2298" } },
		multicall3: {
			address: "0xca11bde05977b3631167028862be2a173976ca11",
			blockCreated: 1376988
		},
		portal: { [sourceId$58]: { address: "0xe93c8cD0D409341205A592f8c4Ac1A5fe5585cfA" } },
		l1StandardBridge: { [sourceId$58]: { address: "0xfA6D8Ee5BE770F84FC001D098C4bD604Fe01284a" } }
	},
	testnet: true,
...
var celoAlfajores = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		disputeGameFactory: { [sourceId$53]: {
			address: "0xE28AAdcd9883746c0e5068F58f9ea06027b214cb",
			blockCreated: 2411324
		} },
		l2OutputOracle: { [sourceId$53]: {
			address: "0x4a2635e9e4f6e45817b1D402ac4904c1d1752438",
			blockCreated: 2411324
		} },
		l1StandardBridge: { [sourceId$53]: {
...
var celoSepolia = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		disputeGameFactory: { [sourceId$52]: {
			address: "0x57c45d82d1a995f1e135b8d7edc0a6bb5211cfaa",
			blockCreated: 8825790
		} },
		l1StandardBridge: { [sourceId$52]: {
...
var codex = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		disputeGameFactory: { [sourceId$51]: { address: "0x6A3855dc26e2beA8Ac73f82Cda79f3808B6C6F6C" } },
		portal: { [sourceId$51]: { address: "0x52759C07A759c81BAab28AE1BE5A19e6450959bD" } },
		l1StandardBridge: { [sourceId$51]: { address: "0xa6b1A05a592719B0C8a70c69eac114C48410aDE4" } },
...
var codexTestnet = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		disputeGameFactory: { [sourceId$50]: { address: "0x390e24E8324E56f13A8d48eB938b6f9De24CD205" } },
		portal: { [sourceId$50]: { address: "0x037F161D12c829A9ca4742eEd9371830CA54fcB2" } },
		l1StandardBridge: { [sourceId$50]: { address: "0xCf4df2bDB14C8FDB25FdacCEC10Ce5C4bAEDB3De" } },
		multicall3: { address: "0xca11bde05977b3631167028862be2a173976ca11" }
	},
...
var form = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		addressManager: { [sourceId$47]: { address: "0x15c249E46A2F924C2dB3A1560CF86729bAD1f07B" } },
		l1CrossDomainMessenger: { [sourceId$47]: { address: "0xF333158DCCad1dF6C3F0a3aEe8BC31fA94d9eD5c" } },
		l2OutputOracle: { [sourceId$47]: { address: "0x4ccAAF69F41c5810cA875183648B577CaCf1F67E" } },
		portal: { [sourceId$47]: { address: "0x4E259Ee5F4136408908160dD32295A5031Fa426F" } },
		l1StandardBridge: { [sourceId$47]: { address: "0xdc20aA63D3DE59574E065957190D8f24e0F7B8Ba" } },
		multicall3: { address: "0xcA11bde05977b3631167028862bE2a173976CA11" }
	},
...
var formTestnet = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		addressManager: { [sourceId$46]: { address: "0xd5C38fa934f7fd7477D4800F4f38a1c5BFdF1373" } },
		l1CrossDomainMessenger: { [sourceId$46]: { address: "0x37A68565c4BE9700b3E3Ec60cC4416cAC3052FAa" } },
		l2OutputOracle: { [sourceId$46]: { address: "0x9eA2239E65a59EC9C7F1ED4C116dD58Da71Fc1e2" } },
		portal: { [sourceId$46]: { address: "0x60377e3cE15dF4CCA24c4beF076b60314240b032" } },
		l1StandardBridge: { [sourceId$46]: { address: "0xD4531f633942b2725896F47cD2aFd260b44Ab1F7" } },
		multicall3: { address: "0xcA11bde05977b3631167028862bE2a173976CA11" }
	},
	testnet: true,
...
var fraxtal = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$45]: { address: "0x66CC916Ed5C6C2FA97014f7D1cD141528Ae171e4" } },
		multicall3: { address: "0xca11bde05977b3631167028862be2a173976ca11" },
		portal: { [sourceId$45]: {
			address: "0x36cb65c1967A0Fb0EEE11569C51C2f2aA1Ca6f6D",
			blockCreated: 19135323
		} },
		l1StandardBridge: { [sourceId$45]: {
...
var fraxtalTestnet = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$44]: { address: "0x715EA64DA13F4d0831ece4Ad3E8c1aa013167F32" } },
		multicall3: { address: "0xca11bde05977b3631167028862be2a173976ca11" },
		portal: { [sourceId$44]: {
...
var garnet = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$41]: {
			address: "0xCb8E7AC561b8EF04F2a15865e9fbc0766FEF569B",
			blockCreated: 1274684
		} },
		l1StandardBridge: { [sourceId$41]: {
...
var giwaSepolia = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		disputeGameFactory: { [sourceId$40]: { address: "0x37347caB2afaa49B776372279143D71ad1f354F6" } },
		portal: { [sourceId$40]: { address: "0x956962C34687A954e611A83619ABaA37Ce6bC78A" } },
		l1StandardBridge: { [sourceId$40]: { address: "0x77b2ffc0F57598cAe1DB76cb398059cF5d10A7E7" } }
	},
	testnet: true,
...
var ink = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		...chainConfig$3.contracts,
		disputeGameFactory: { [sourceId$37]: { address: "0x10d7b35078d3baabb96dd45a9143b94be65b12cd" } },
		portal: { [sourceId$37]: { address: "0x5d66c1782664115999c47c9fa5cd031f495d3e4f" } },
		l1StandardBridge: { [sourceId$37]: { address: "0x88ff1e5b602916615391f55854588efcbb7663f0" } }
	},
	testnet: false,
...
var inkSepolia = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		disputeGameFactory: { [sourceId$36]: { address: "0x860e626c700af381133d9f4af31412a2d1db3d5d" } },
		portal: { [sourceId$36]: { address: "0x5c1d29c6c9c8b0800692acc95d700bcb4966a1d7" } },
		l1StandardBridge: { [sourceId$36]: { address: "0x33f60714bbd74d62b66d79213c348614de51901c" } }
	},
	testnet: true,
...
var lisk = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$35]: { address: "0x113cB99283AF242Da0A0C54347667edF531Aa7d6" } },
		portal: { [sourceId$35]: { address: "0x26dB93F8b8b4f7016240af62F7730979d353f9A7" } },
		l1StandardBridge: { [sourceId$35]: { address: "0x2658723Bf70c7667De6B25F99fcce13A16D25d08" } }
	},
	sourceId: sourceId$35
});
//#endregion
//#region node_modules/viem/_esm/chains/definitions/liskSepolia.js
var sourceId$34 = 11155111;
var liskSepolia = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$34]: { address: "0xA0E35F56C318DE1bD5D9ca6A94Fe7e37C5663348" } },
		multicall3: { address: "0xca11bde05977b3631167028862be2a173976ca11" },
		portal: { [sourceId$34]: { address: "0xe3d90F21490686Ec7eF37BE788E02dfC12787264" } },
		l1StandardBridge: { [sourceId$34]: { address: "0x1Fb30e446eA791cd1f011675E5F3f5311b70faF5" } }
	},
	testnet: true,
...
var metalL2 = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$33]: { address: "0x3B1F7aDa0Fcc26B13515af752Dd07fB1CAc11426" } },
		multicall3: {
			address: "0xcA11bde05977b3631167028862bE2a173976CA11",
			blockCreated: 0
		},
		portal: { [sourceId$33]: { address: "0x3F37aBdE2C6b5B2ed6F8045787Df1ED1E3753956" } },
		l1StandardBridge: { [sourceId$33]: { address: "0x6d0f65D59b55B0FEC5d2d15365154DcADC140BF3" } }
	},
...
var mode = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$32]: { address: "0x4317ba146D4933D889518a3e5E11Fe7a53199b04" } },
		portal: { [sourceId$32]: { address: "0x8B34b14c7c7123459Cf3076b8Cb929BE097d0C07" } },
		l1StandardBridge: { [sourceId$32]: { address: "0x735aDBbE72226BD52e818E7181953f42E3b0FF21" } }
	},
	sourceId: sourceId$32
});
//#endregion
//#region node_modules/viem/_esm/chains/definitions/modeTestnet.js
var sourceId$31 = 11155111;
...
var opBNB = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$30]: { address: "0x153CAB79f4767E2ff862C94aa49573294B13D169" } },
		portal: { [sourceId$30]: { address: "0x1876EA7702C0ad0C6A2ae6036DE7733edfBca519" } },
		l1StandardBridge: { [sourceId$30]: { address: "0xF05F0e4362859c3331Cb9395CBC201E3Fa6757Ea" } }
	},
...
var opBNBTestnet = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$29]: { address: "0xFf2394Bb843012562f4349C6632a0EcB92fC8810" } },
		portal: { [sourceId$29]: { address: "0x4386C8ABf2009aC0c263462Da568DD9d46e52a31" } },
		l1StandardBridge: { [sourceId$29]: { address: "0x677311Fd2cCc511Bbc0f581E8d9a07B033D5E840" } }
	},
	testnet: true,
...
var optimismGoerli = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...chainConfig$3.contracts,
		l2OutputOracle: { [sourceId$28]: { address: "0xE6Dfba0953616Bacab0c9A8ecb3a9BBa77FC15c0" } },
		multicall3: {
			address: "0xca11bde05977b3631167028862be2a173976ca11",
			blockCreated: 49461
		},
		portal: { [sourceId$28]: { address: "0x5b47E1A08Ea6d985D6649300584e6722Ec4B1383" } },
		l1StandardBridge: { [sourceId$28]: { address: "0x636Af16bf2f682dD3109e60102b8E1A089FedAa8" } }
	},
	testnet: true,
...
function extractAddress(options) {
	...
	return fromPublicKey$1(extractPublicKey(options));
}
/**
...
function deserialize$1(value) {
	...
	if (size(serialized) === 65) {
		const signature = fromHex$2(serialized);
		assert$3(signature);
		return {
			signature,
			type: "secp256k1"
	...
	if (typeId === serializedWebAuthnType) {
		...
		for (let split = 37; split < webauthnDataSize; split++) {
			const potentialJson = toString$1(slice(webauthnData, split));
	...
	return {
		...
		...type === "keychain" ? {
			...
			...!(typeof value === "object" && "keyId" in value && value.keyId) ? (() => {
				...
				if (inner.type === "p256" || inner.type === "webAuthn") return { keyId: fromPublicKey$1(inner.publicKey) };
				if (inner.type === "secp256k1" && options?.payload) return { keyId: fromPublicKey$1(recoverPublicKey({
...
function fromRpc$3(envelope) {
	...
	if (envelope.type === "keychain" || "userAddress" in envelope && "signature" in envelope) return {
		...
		inner: fromRpc$3(envelope.signature),
...
function serialize$2(envelope, options = {}) {
	...
	if (type === "secp256k1") return concat(toHex$2(envelope.signature), options.magic ? magicBytes : "0x");
	if (type === "p256") {
		const p256 = envelope;
		return concat(serializedP256Type, fromNumber(p256.signature.r, { size: 32 }), fromNumber(p256.signature.s, { size: 32 }), fromNumber(p256.publicKey.x, { size: 32 }), fromNumber(p256.publicKey.y, { size: 32 }), fromNumber(p256.prehash ? 1 : 0, { size: 1 }), options.magic ? magicBytes : "0x");
	}
	if (type === "webAuthn") {
		const webauthn = envelope;
		return concat(serializedWebAuthnType, concat(webauthn.metadata.authenticatorData, fromString$1(webauthn.metadata.clientDataJSON)), fromNumber(webauthn.signature.r, { size: 32 }), fromNumber(webauthn.signature.s, { size: 32 }), fromNumber(webauthn.publicKey.x, { size: 32 }), fromNumber(webauthn.publicKey.y, { size: 32 }), options.magic ? magicBytes : "0x");
	}
	if (type === "keychain") {
		const keychain = envelope;
		return concat(keychain.version === "v1" ? serializedKeychainType : serializedKeychainV2Type, keychain.userAddress, serialize$2(keychain.inner), options.magic ? magicBytes : "0x");
	}
	throw new CoercionError({ envelope });
}
/**
...
function toRpc$3(envelope) {
	...
	if (type === "webAuthn") {
		const webauthn = envelope;
		const webauthnData = concat(webauthn.metadata.authenticatorData, fromString$1(webauthn.metadata.clientDataJSON));
		return {
	...
	if (type === "keychain") {
		...
		return {
			...
			signature: toRpc$3(keychain.inner),
...
function verify(signature, parameters) {
	...
	const address = (() => {
		...
		if (parameters.publicKey) return fromPublicKey$1(parameters.publicKey);
	})();
	...
	if (envelope.type === "p256") {
		if (!isEqual$1(fromPublicKey$1(envelope.publicKey), address)) return false;
		return verify$3({
...
function from$2(authorization, options = {}) {
	if (typeof authorization.chainId === "string") return fromRpc$2(authorization);
	const resolved = {
		...authorization,
...
function fromRpc$2(authorization) {
	...
	const signature = fromRpc$3(authorization.signature);
	return {
		address,
...
function fromRpcList(authorizationList) {
	return authorizationList.map((x) => fromRpc$2(x));
}
/**
...
function toRpcList(authorizationList) {
	return authorizationList.map((x) => toRpc$2(x));
}
/**
...
function from$1(authorization, options = {}) {
	if ("keyId" in authorization) return fromRpc$1(authorization);
...
* @returns A signed {@link ox#AuthorizationTempo.AuthorizationTempo}.
*/
function fromRpc$1(authorization) {
...
function fromTuple(tuple) {
	...
	const args = {
		address: keyId,
		expiry: typeof expiry !== "undefined" ? hexToNumber(expiry) : void 0,
		type: keyType,
		chainId: chainId === "0x" ? 0n : toBigInt(chainId),
		...typeof expiry !== "undefined" ? { expiry: hexToNumber(expiry) } : {},
		...typeof limits !== "undefined" ? { limits: limits.map(([token, limit]) => ({
			token,
...
*/
function toRpc$1(authorization) {
...
function toTuple(authorization) {
	...
	const limitsValue = limits?.map((limit) => [limit.token, bigintToHex(limit.limit)]);
	return [[
		bigintToHex(chainId),
		type,
		address,
		typeof expiry === "number" || limitsValue ? numberToHex(expiry ?? 0) : void 0,
		limitsValue
	].filter(Boolean), ...signature ? [signature] : []];
}
...
function toAddress(tokenId) {
	...
	return concat(tip20Prefix, fromNumber(tokenId, { size: 18 }));
}
//#endregion
...
function fromRpc(transaction, _options = {}) {
	if (!transaction) return null;
	const transaction_ = fromRpc$4(transaction);
	...
	if (transaction.signature) transaction_.signature = fromRpc$3(transaction.signature);
	...
	if (transaction.keyAuthorization) transaction_.keyAuthorization = fromRpc$1(transaction.keyAuthorization);
...
function toRpc(request) {
	...
	if (request.keyAuthorization) request_rpc.keyAuthorization = toRpc$1(request.keyAuthorization);
...
function assert(envelope) {
	...
	if (calls) {
		for (const call of calls) if (call.to) assert$2(call.to, { strict: false });
	}
...
function deserialize(serialized) {
	...
	if (!(transactionArray.length === 13 || transactionArray.length === 14 || transactionArray.length === 15)) throw new InvalidSerializedError$1({
		attributes: {
			authorizationList,
			chainId,
			maxPriorityFeePerGas,
	...
	if (validate(nonce)) transaction.nonce = nonce === "0x" ? 0n : BigInt(nonce);
	if (validate(maxFeePerGas) && maxFeePerGas !== "0x") transaction.maxFeePerGas = BigInt(maxFeePerGas);
	if (validate(maxPriorityFeePerGas) && maxPriorityFeePerGas !== "0x") transaction.maxPriorityFeePerGas = BigInt(maxPriorityFeePerGas);
	if (validate(nonceKey)) transaction.nonceKey = nonceKey === "0x" ? 0n : BigInt(nonceKey);
	if (validate(validBefore) && validBefore !== "0x") transaction.validBefore = Number(validBefore);
	if (validate(validAfter) && validAfter !== "0x") transaction.validAfter = Number(validAfter);
	if (validate(feeToken) && feeToken !== "0x") transaction.feeToken = feeToken;
	...
	if (accessList?.length !== 0 && accessList !== "0x") transaction.accessList = fromTupleList$1(accessList);
	if (authorizationList?.length !== 0 && authorizationList !== "0x") transaction.authorizationList = fromTupleList(authorizationList);
...
function serialize$1(envelope, options = {}) {
	...
	const feePayerSignatureOrSender = (() => {
		...
		if (options.format === "feePayer" && signature) {
			...
			if (sig.type === "p256" || sig.type === "webAuthn") return fromPublicKey$1(sig.publicKey);
	...
	return concat(options.format === "feePayer" ? feePayerMagic : serializedType, fromHex$3(serialized));
}
/**
...
function getSignPayload(envelope, options = {}) {
	...
	if (options.from) return keccak256(concat("0x04", sigHash, resolve(options.from)));
	return sigHash;
}
/**
...
async function serialize(transaction, signature) {
	if (!isTempo(transaction)) {
		...
		if (signature && "type" in signature) {
			...
			return serializeTransaction$1(transaction, {
...
function formatTransactionRequest(r, action) {
	...
	const [keyType, keyData] = (() => {
		...
		if (!type) return [void 0, void 0];
		if (type === "webAuthn") return ["webAuthn", `0x${"ff".repeat(1400)}`];
	...
	return {
		...
		...keyId ? { keyId } : {},
		...keyType ? { keyType } : {},
		...request.feePayer ? { feePayer: typeof request.feePayer === "object" ? parseAccount(request.feePayer) : request.feePayer } : {}
	};
}
//#endregion
...
var chainConfig = {
	...
	prepareTransactionRequest: [async (r, { phase }) => {
		const request = r;
	...
	async verifyHash(client, parameters) {
		...
		if (typeof signature === "string" && signature.endsWith("0x7777777777777777777777777777777777777777777777777777777777777777".slice(2))) {
			const envelope = deserialize$1(signature);
		...
		return await getAction(client, verifyHash, "verifyHash")({
			...parameters,
			chain: null
		});
	}
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/tempoAndantino.js
var tempoAndantino = /* @__PURE__ */ defineChain({
	...chainConfig,
	id: 42429,
	blockExplorers: { default: {
		name: "Tempo Explorer",
...
//#endregion
//#region node_modules/viem/_esm/chains/definitions/zircuitTestnet.js
...
var zircuitTestnet = /* @__PURE__ */ defineChain({
	...
	contracts: {
		...
		l2OutputOracle: { [sourceId$2]: { address: "0x740C2dac453aEf7140809F80b72bf0e647af8148" } },
		portal: { [sourceId$2]: { address: "0x787f1C8c5924178689E0560a43D848bF8E54b23e" } },
		l1StandardBridge: { [sourceId$2]: { address: "0x0545c5fe980098C16fcD0eCB5E79753afa6d9af9" } }
	},
	testnet: true
});
//#endregion
...
//#endregion
export { wemix as $, mantleTestnet as $a, flare as $c, arbitrumNova as $d, oasisTestnet as $i, degen as $l, skaleEuropa as $n, juneoSGD1Chain as $o, quaiTestnet as $r, hederaTestnet as $s, taikoHekla as $t, bronosTestnet as $u, xrplevmTestnet as A, metisSepolia as Aa, garnet as Ac, basecampTestnet as Ad, pgnTestnet as Ai, elastosTestnet as Al, sophon as An, l3xTestnet as Ao, saga as Ar, immutableZkEvm as As, thunderTestnet as At, confluxESpace as Au, xgr as B, merlin as Ba, forta as Bc, astarZkyoto as Bd, optopia as Bi, dymension as Bl, snaxTestnet as Bn, kii as Bo, rollux as Br, humanity as Bs, tempoDevnet as Bt, chang as Bu, zetachain as C, modeTestnet as Ca, glideL2Protocol as Cc, berachainTestnetbArtio as Cd, zeroGGalileoTestnet as Cf, plinga as Ci, enuls as Cl, statusSepolia as Cn, linea as Co, scroll as Cr, ink as Cs, tronShasta as Ct, cpchain as Cu, yooldoVerseTestnet as D, mint as Da, genesys as Dc, beamTestnet as Dd, plasma as Di, elysiumTestnet as Dl, sovaSepolia as Dn, lensTestnet as Do, sapphire as Dr, initVerse as Ds, treasure as Dt, coreTestnet1 as Du, zenchainTestnet as E, mintSepoliaTestnet as Ea, giwaSepoliaPreconf as Ec, bearNetworkChainMainnet as Ed, plasmaDevnet as Ei, energy as El, spicy as En, lestnet as Eo, sapphireTestnet as Er, initVerseGenesis as Es, treasureTopaz as Et, coreTestnet2 as Eu, xphereMainnet as F, metalL2 as Fa, funkiSepolia as Fc, autheoTestnet as Fd, palm as Fi, edgeware as Fl, songbird as Fn, koi as Fo, rootstock as Fr, hychainTestnet as Fs, thaiChain as Ft, classic as Fu, worldLand as G, megaethTestnet as Ga, fluentDevnet as Gc, artheraTestnet as Gd, oortMainnetDev as Gi, dosChain as Gl, skaleBaseSepoliaTestnet as Gn, kardiaChain as Go, rei as Gr, hpb as Gs, telcoinTestnet as Gt, bxnTestnet as Gu, xdc as H, memecore as Ha, forma as Hc, astar as Hd, openledger as Hi, dreyerxTestnet as Hl, sketchpad as Hn, kavaTestnet as Ho, riseTestnet as Hr, huddle01Mainnet as Hs, tempo as Ht, celoAlfajores as Hu, xoneTestnet as I, metadium as Ia, funkiMainnet as Ic, auroria as Id, otimDevnet as Ii, edgelessTestnet as Il, soneiumMinato as In, klaytnBaobab as Io, rootPorcini as Ir, hychain as Is, ternoa as It, citreaTestnet as Iu, wmcTestnet as J, matchainTestnet as Ja, fluence as Jc, areum as Jd, omni as Ji, dogechain as Jl, skaleNebulaTestnet as Jn, kairos as Jo, reddio as Jr, henesys as Js, taraxa as Jt, btr as Ju, worldchainSepolia as K, megaeth as Ka, fluenceTestnet as Kc, arthera as Kd, oneWorld as Ki, donatuz as Kl, skaleBase as Kn, kakarotStarknetSepolia as Ko, redstone as Kr, horizenTestnet as Ks, teaSepolia as Kt, bxn as Ku, xoneMainnet as L, metachainIstanbul as La, fraxtalTestnet as Lc, auroraTestnet as Ld, orderlySepolia as Li, edgeless as Ll, soneium as Ln, klaytn as Lo, root as Lr, humanodeTestnet5 as Ls, tenet as Lt, citrea as Lu, xrplevm as M, metis as Ma, fusion as Mc, b3Sepolia as Md, peaq as Mi, eduChainTestnet as Ml, sonicBlazeTestnet as Mn, krown as Mo, rss3Sepolia as Mr, icbNetwork as Ms, thetaTestnet as Mt, coinbit as Mu, xrOne as N, meterTestnet as Na, fuseSparknet as Nc, b3 as Nd, paseoPassetHub as Ni, eduChain as Nl, sonic as Nn, kromaSepolia as No, rss3 as Nr, hyperliquidEvmTestnet as Ns, theta as Nt, codexTestnet as Nu, yooldoVerse as O, mevTestnet as Oa, geist as Oc, beam as Od, planq as Oi, electroneumTestnet as Ol, sova as On, lens as Oo, sanko as Or, inEVM as Os, tomb as Ot, coreDao as Ou, xphereTestnet as P, meter as Pa, fuse as Pc, avalancheFuji as Pd, palmTestnet as Pi, edgewareTestnet as Pl, songbirdTestnet as Pn, kroma as Po, rootstockTestnet as Pr, hyperEvm as Ps, that as Pt, codex as Pu, wemixTestnet as Q, mantraDuKongEVMTestnet as Qa, flareTestnet as Qc, arcTestnet as Qd, oasys as Qi, dfk as Ql, skaleEuropaTestnet as Qn, juneoSocotraTestnet as Qo, reactiveTestnet as Qr, hela as Qs, taikoHoodi as Qt, bscGreenfield as Qu, xLayerTestnet as R, metachain as Ra, fraxtal as Rc, aurora as Rd, orderly as Ri, edexaTestnet as Rl, somniaTestnet as Rn, kinto as Ro, ronin as Rr, humanode as Rs, tempoModerato as Rt, chips as Ru, zetachainAthensTestnet as S, monad as Sa, gnosisChiado as Sc, bevmMainnet as Sd, zeroGMainnet as Sf, plume as Si, eon as Sl, step as Sn, lineaGoerli as So, scrollSepolia as Sr, inkSepolia as Ss, ubiq as St, crab as Su, zeniq as T, mitosisTestnet as Ta, giwaSepolia as Tc, bearNetworkChainTestnet as Td, plasmaTestnet as Ti, eni as Tl, stable as Tn, lightlinkPegasus as To, satoshiVM as Tr, injective as Ts, tron as Tt, corn as Tu, xaiTestnet as U, meld as Ua, form as Uc, assetChainTestnet as Ud, opBNBTestnet as Ui, dreyerxMainnet as Ul, skaleTitanTestnet as Un, kava as Uo, rise as Ur, hppSepolia as Us, telosTestnet as Ut, canto as Uu, xdcTestnet as V, formicarium as Va, formTestnet as Vc, astarZkEVM as Vd, optimismGoerli as Vi, dustboyIoT as Vl, snax as Vn, kcc as Vo, rivalz as Vr, huddle01Testnet as Vs, tempoAndantino as Vt, celoSepolia as Vu, xai as W, mekong as Wa, fluentTestnet as Wc, assetChain as Wd, opBNB as Wi, dosChainTestnet as Wl, skaleTitan as Wn, karura as Wo, reyaNetwork as Wr, hpp as Ws, telos as Wt, cannon as Wu, whitechain as X, mapProtocol as Xa, flowPreviewnet as Xc, areonNetwork as Xd, okc as Xi, disChain as Xl, skaleHumanProtocol as Xn, juneoUSDT1Chain as Xo, redbellyMainnet as Xr, hemi as Xs, taikoKatla as Xt, bsquared as Xu, whitechainTestnet as Y, matchain as Ya, flowTestnet as Yc, areonNetworkTestnet as Yd, omax as Yi, dodochainTestnet as Yl, skaleNebula as Yn, kaia as Yo, redbellyTestnet as Yr, hemiSepolia as Ys, taikoTestnetSepolia as Yt, bsquaredTestnet as Yu, westendAssetHub as Z, mantraEVM as Za, flowMainnet as Zc, arenaz as Zd, odysseyTestnet as Zi, diode as Zl, skaleExorde as Zn, juneoUSD1Chain as Zo, real as Zr, heliosTestnet as Zs, taikoJolnir as Zt, bscTestnet as Zu, zircuitGarfieldTestnet as _, moonriver as _a, goerli as _c, bitkubTestnet as _d, abstractTestnet as _f, polterTestnet as _i, etherlinkShadownetTestnet as _l, stratis as _n, loadAlphanet as _o, shapeSepolia as _r, iSunCoin as _s, unichain as _t, cronos as _u, zksyncSepoliaTestnet as a, newton as aa, harmonyOne as ac, boolBetaMainnet as ad, ancient8 as af, pumpfiTestnet as ai, fantomTestnet as al, swissdlt as an, mandala as ao, sixProtocol as ar, juneoDOGE1Chain as as, victionTestnet as at, datahavenTestnet as au, zilliqa as b, moonbaseAlpha as ba, gobi as bc, birdlayer as bd, fireChain as bf, plumeMainnet as bi, eosTestnet as bl, storyAeneid as bn, lineaTestnet as bo, seismicDevnet as br, iotaTestnet as bs, ultraTestnet as bt, creditCoin3Devnet as bu, zksyncLocalHyperchain as c, neonMainnet as ca, haqqMainnet as cc, boba as cd, alienx as cf, premiumBlockTestnet as ci, exsatTestnet as cl, swanSaturnTestnet as cn, luxeports as co, silentData as cr, juneo as cs, vechain as ct, dailyNetwork as cu, zksync as d, near as da, guruTestnet as dc, blast as dd, aioz as df, polynomialSepolia as di, excelonMainnet as dl, surgeTestnet as dn, lumiaTestnet as do, shimmer as dr, jocTestnet as ds, vana as dt, curtis as du, nomina as ea, hederaPreviewnet as ec, bronos as ed, arbitrumGoerli as ef, quai as ei, flame as el, taiko as en, mantleSepoliaTestnet as eo, skaleCryptoColosseum as er, juneomBTC1Chain as es, weaveVMAlphanet as et, defichainEvmTestnet as eu, zkLinkNovaSepoliaTestnet as f, nautilus as fa, guruNetwork as fc, bitTorrentTestnet as fd, agungTestnet as ff, polynomial as fi, evmosTestnet as fl, superseedSepolia as fn, lumiaMainnet as fo, shiden as fr, jocMainnet as fs, unreal as ft, crossfi as fu, zircuitTestnet as g, morph as ga, graphite as gc, bitlayer as gd, acala as gf, polygonMumbai as gi, etherlinkTestnet as gl, subtensorEvm as gn, localhost as go, shardeum as gr, jasmyChain as gs, unichainSepolia as gt, cronosTestnet as gu, zkFair as h, morphHolesky as ha, graphiteTestnet as hc, bitlayerTestnet as hd, acria as hf, polygonZkEvm as hi, ethernity as hl, superlumio as hn, loop as ho, shardeumSphinx as hr, jasmyChainTestnet as hs, unique as ht, cronoszkEVM as hu, zkXPLA as i, nexi as ia, hashkey as ic, botanix as id, ancient8Sepolia as if, pyrope as ii, fibo as il, syscoin as in, manta as io, skaleBlockBrawlers as ir, juneoEUR1Chain as is, vision as it, dbkchain as iu, xrplevmDevnet as j, metisGoerli as ja, fusionTestnet as jc, bahamut as jd, pgn as ji, elastos as jl, sonicTestnet as jn, l3x as jo, saakuru as jr, idchain as js, thunderCore as jt, coinex as ju, xrSepolia as k, mev as ka, gatechain as kc, baseGoerli as kd, phoenix as ki, electroneum as kl, sophonTestnet as kn, lavita as ko, saigon as kr, immutableZkEvmTestnet as ks, tiktrixTestnet as kt, confluxESpaceTestnet as ku, zksyncLocalCustomHyperchain as l, neonDevnet as la, happychainTestnet as lc, bob as ld, alephZeroTestnet as lf, potosTestnet as li, exsat as ll, swanProximaTestnet as ln, lumozTestnet as lo, sidraChain as lr, jovaySepolia as ls, vanar as lt, cyberTestnet as lu, zkFairTestnet as m, morphSepolia as ma, gravity as mc, bitrock as md, adf as mf, polygonZkEvmCardona as mi, etp as ml, superposition as mn, lukso as mo, shibarium as mr, jbc as ms, uniqueOpal as mt, cronoszkEVMTestnet as mu, zoraSepolia as n, nibiru as na, haustTestnet as nc, bounceBit as nd, apexTestnet as nf, qMainnet as ni, filecoinCalibration as nl, tac as nn, mantaTestnet as no, skaleCalypsoTestnet as nr, juneoLINK1Chain as ns, wanchain as nt, dchainTestnet as nu, zksyncLocalNode as o, neoxT4 as oa, hardhat as oc, bobSepolia as od, alpenTestnet as of, pulsechainV4 as oi, fantomSonicTestnet as ol, swellchainTestnet as on, lyra as oo, siliconSepolia as or, juneoDAI1Chain as os, viction as ot, darwinia as ou, zkLinkNova as p, nahmii as pa, gunz as pc, bitTorrent as pd, adi as pf, polygonZkEvmTestnet as pi, evmos as pl, superseed as pn, luksoTestnet as po, shibariumTestnet as pr, jbcTestnet as ps, uniqueQuartz as pt, crossbell as pu, worldchain as q, mchVerse as qa, fluenceStage as qc, artelaTestnet as qd, omniOmega as qi, domaTestnet as ql, skaleRazor as qn, kakarotSepolia as qo, reddioSepolia as qr, holesky as qs, taraxaTestnet as qt, btrTestnet as qu, zkXPLATestnet as r, nexilix as ra, hashkeyTestnet as rc, botanixTestnet as rd, apeChain as rf, ql1 as ri, filecoin as rl, syscoinTestnet as rn, mantaSepoliaTestnet as ro, skaleCalypso as rr, juneoGLD1Chain as rs, visionTestnet as rt, dchain as ru, zksyncLocalHyperchainL1 as s, neoxMainnet as sa, haqqTestedge2 as sc, bobaSepolia as sd, alienxHalTestnet as sf, pulsechain as si, fantom as sl, swellchain as sn, lycan as so, silicon as sr, juneoBCH1Chain as ss, velas as st, dailyNetworkTestnet as su, zoraTestnet as t, nitrographTestnet as ta, hedera as tc, bounceBitTestnet as td, apollo as tf, qTestnet as ti, filecoinHyperspace as tl, tacSPB as tn, mantle as to, skaleCryptoBlades as tr, juneoLTC1Chain as ts, wanchainTestnet as tt, defichainEvm as tu, zksyncInMemoryNode as u, nearTestnet as ua, ham as uc, blastSepolia as ud, alephZero as uf, potos as ui, expanse as ul, swan as un, lumoz as uo, shimmerTestnet as ur, jovay as us, vanaMoksha as ut, cyber as uu, zircuit as v, moonbeamDev as va, godwoken as vc, bitkub as vd, abstract as vf, plumeTestnet as vi, etherlink as vl, storyTestnet as vn, liskSepolia as vo, shape as vr, iotexTestnet as vs, ultronTestnet as vt, creditCoin3Testnet as vu, zeroNetwork as w, mode as wa, glideL1Protocol as wc, berachainTestnet as wd, zeroG as wf, playfiAlbireo as wi, eniTestnet as wl, stableTestnet as wn, lightlinkPhoenix as wo, satoshiVMTestnet as wr, injectiveTestnet as ws, tronNile as wt, cornTestnet as wu, zhejiang as x, monadTestnet as xa, goat as xc, bifrost as xd, zeroGTestnet as xf, plumeDevnet as xi, eos as xl, story as xn, lineaSepolia as xo, sei as xr, iota as xs, ultra as xt, creatorTestnet as xu, zilliqaTestnet as y, moonbeam as ya, goChain as yc, bitgert as yd, abey as yf, plumeSepolia as yi, eteria as yl, storyOdyssey as yn, lisk as yo, seiTestnet as yr, iotex as ys, ultron as yt, creditCoin3Mainnet as yu, xLayer as z, merlinErigonTestnet as za, foundry as zc, atletaOlympia as zd, optopiaTestnet as zi, edexa as zl, somnia as zn, kiiTestnetOro as zo, rolluxTestnet as zr, humanityTestnet as zs, tempoLocalnet as zt, chiliz as zu };

//# sourceMappingURL=chains-Dkl0rPNC.js.map